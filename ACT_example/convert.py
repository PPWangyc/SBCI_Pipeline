import os

# list folders in a directory
def get_folder_list(path):
    return [f for f in os.listdir(path) if os.path.isdir(os.path.join(path, f))]
ACT_PATH = '/home/yanchen/Data/ACT'
CONFIG_PATH = os.path.join(ACT_PATH,'bids', 'config.json')
OUTPUT_BIDS_PATH = os.path.join('/home/yanchen/Data/ACT_BIDS')
session='01'
dicom_dirs = get_folder_list(os.path.join(ACT_PATH, 'dicom'))

print(len(dicom_dirs))
for dicom_dir in dicom_dirs:
    screen_id = dicom_dir.split('_')[-1]
    dicom_dir_path = os.path.join(ACT_PATH, 'dicom', dicom_dir)
    command = 'sh convert_step1.sh {} {} {} {} {}'.format(dicom_dir_path, screen_id, session,CONFIG_PATH, OUTPUT_BIDS_PATH)
    print(command)
    os.system(command)
    